...
| 항목 | 요구사항 |
|------|----------|
| OS | Windows 10/11 (`start_stock.bat` 기준 — 다른 OS는 개발자용 실행으로 가능) |
| Python | **3.10 ~ 3.12** (3.13은 OpenDartReader 0.3.x 필요 — requirements 주석 참조). 설치 시 "Add Python to PATH" 체크 또는 `py` 런처 |
| 인터넷 | 시세·재무 데이터 조회 (지연/EOD) |
| 디스크 | 수백 MB (venv + parquet 캐시) |

**API 키 (선택 — 없으면 해당 기능만 비활성, 나머지는 정상 동작)**
| 키 | 발급처 | 잠금 해제되는 기능 |
|----|--------|--------------------|
| `DART_API_KEY` | [DART 오픈API](https://opendart.fss.or.kr) | KR 상세 재무제표·공시 |
| `GEMINI_API_KEY` | [Google AI Studio](https://aistudio.google.com) | AI 서술 코멘트 7종 (숫자엔 관여 안 함) |
| `KIS_APP_KEY/SECRET` | [KIS Developers](https://apiportal.koreainvestment.com) | KIS 시세·모의투자 잔고/주문·자동매매 (아래 발급 절차) |

**네트워크 포트 (아웃바운드)** — 학교·회사망에서 자주 차단됨:
| 용도 | 포트 | 차단 시 증상 |
|------|------|--------------|
| 일반 데이터 (FDR·yfinance·pykrx·DART·Gemini) | 443 | — (거의 항상 열려 있음) |
| KIS REST (실전/모의) | **9443 / 29443** | `/api/kis/status`가 `connected: false` |
| KIS WebSocket (실전/모의, 향후) | **21000 / 31000** | 실시간 스트림 불가 |

> 포트 차단 여부 빠른 확인: `Test-NetConnection portquiz.net -Port 9443` (portquiz는 모든 포트 응답 서버 —
> 여기가 실패하면 내 네트워크가 차단한 것). 차단망에서는 KIS 기능만 비활성화되고 나머지는 정상 동작한다.

**기능별 필요 조건 매트릭스**
| 기능 | 키 없음 | +DART | +GEMINI | +KIS(모의) |
|------|:-:|:-:|:-:|:-:|
| 차트·지표·스크리너·추천·백테스트·과거 재생 | ✅ | ✅ | ✅ | ✅ |
| KR 재무제표·공시 요약 | — | ✅ | ✅(요약) | ✅ |
| AI 서술 코멘트 | — | — | ✅ | ✅ |
| KIS 잔고·모의 주문·자동매매 | — | — | — | ✅ (`KIS_MOCK=true`) |
| 실계좌 주문 | 기본 차단 — `KIS_ALLOW_REAL_ORDERS=true` 명시 시에만 (비권장) | | | |
...
| 매매 | **포지션 계산기**(R 기반 사이징·ATR/가격/% 손절) + **매매 저널**(P&L·R·승률·기대값) + **자동매매(모의 전용)** + **과거 재생**(전략 검증·규칙별 귀인) |
